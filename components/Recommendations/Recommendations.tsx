...
Проанализируй, склоняюсь ли я больше к логико-структурным решениям или интуитивно-абстрактным, и как это влияет на мою жизнь.`,
                brainstorm: `Ты — мой внешний визуализатор. Я буду описывать свои идеи как структуры или концепции, а ты помогай мне «видеть» их, создавая визуально-богатые описания или схемы, которые я не могу сгенерировать самостоятельно.
...
Какие возможные точки трения или недопонимания могут возникать с другими людьми? Что они могут не понимать в моей манере общения, и что я могу упускать в их? Как мне лучше адаптировать свою коммуникацию?`
    ...
    return (
        <div className="min-h-screen bg-[#0D0D0D] text-white p-6 md:p-12 font-sans selection:bg-indigo-500/30 -mx-6 md:-mx-12 -my-24 overflow-x-hidden">
            <div className="max-w-6xl mx-auto mb-20 animate-fade-in pt-24 text-left">
                <div className="flex flex-col md:flex-row justify-between items-start gap-6 mb-6">
                    <div>
                        ...
                        <p className="text-gray-400 max-w-2xl leading-relaxed text-sm md:text-lg">
                            {ui.jumpToGeminiDesc.split('.')[0]}. {ui.jumpToGeminiDesc.split('.')[1] || ''}
                        </p>
                    </div>
                    <div className="bg-[#1A1A1A] border border-white/10 px-6 py-3 rounded-full flex items-center gap-4 shadow-xl">
                    ...
                    <div className="mt-12 bg-indigo-600/10 border border-indigo-500/30 rounded-[2.5rem] p-10 flex flex-col md:flex-row items-center justify-between gap-8 overflow-hidden relative group">
                        ...
                        <div className="flex items-center gap-8 relative z-10 text-left">
                            ...
                            <div>
                                ...
                                <p className="text-gray-400 text-sm md:text-base max-w-xl">{ui.recommendationsCTADesc}</p>
                            </div>
                        </div>
                        <button
                            onClick={() => navigate('/')}
            ...
            {isLocked ? (
                <div className="max-w-6xl mx-auto bg-[#1A1A1A] border border-white/5 border-dashed rounded-[40px] p-24 text-center shadow-2xl relative overflow-hidden">
                    <div className="absolute inset-0 bg-indigo-500/[0.02] pointer-events-none" />
                    <div className="relative z-10 max-w-md mx-auto">
                        <div className="w-20 h-20 bg-[#262626] border border-white/10 rounded-full flex items-center justify-center mx-auto mb-8 text-gray-500 shadow-inner">
                ...
                <div className="max-w-6xl mx-auto space-y-24 pb-24">
                    {sections.map((section) => (
                        <div key={section.id} id={section.id} className="scroll-mt-32">
                            <div className="mb-10 text-left">
                                ...
                                <p className="text-gray-500 text-lg ml-6">{section.subline}</p>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 ml-6">
                                {section.cards.map((card) => (
                                    <div 
                                        ...
                                        className={`group bg-[#1A1A1A] border-l-4 ${section.color} border-t border-r border-b border-white/5 rounded-[2.5rem] p-8 cursor-pointer transition-all duration-300 hover:bg-[#1f1f1f] hover:border-white/10 flex flex-col min-h-[420px] relative text-left shadow-lg hover:shadow-2xl hover:-translate-y-1`}
                                    >
                                        ...
                                        <div className="flex-1">
                                            <div className={`w-14 h-14 bg-[#262626] rounded-2xl flex items-center justify-center mb-8 border border-white/5 group-hover:scale-110 transition-transform duration-500 ${section.iconColor}`}>
                                            ...
                                            <div className="relative bg-[#0D0D0D] border border-white/5 rounded-2xl p-5 group-hover:border-white/10 transition-colors h-[120px] overflow-hidden">
                                                <p className="text-gray-500 text-xs font-mono leading-relaxed line-clamp-4">
                                                    {card.template()}
                                                </p>
                                        ...
                                        <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-between">
                                            ...
                                            <div className={`w-10 h-10 rounded-full border border-white/10 flex items-center justify-center bg-[#262626] group-hover:bg-white group-hover:text-black transition-all shadow-lg`}>
                    ...
                    {/* Custom Prompt Block */}
                    <div className="bg-[#1A1A1A] border border-white/5 rounded-[40px] p-10 md:p-16 shadow-2xl relative overflow-hidden group text-left border-t-indigo-500/30">
                            ...
                            <div className="flex-1 space-y-6 w-full">
                                ...
                                <div className="relative">
                                    <textarea
                                        ...
                                        className="w-full bg-[#0D0D0D] border border-white/10 rounded-[2rem] p-8 focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500/50 outline-none min-h-[180px] text-lg text-white transition-all placeholder-gray-700 shadow-inner"
                                    />
            ...
            {/* Modal Window */}
            {selectedCategory && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0D0D0D]/95 backdrop-blur-2xl animate-in fade-in duration-500">
                    <div 
                        className="absolute inset-0"
                        onClick={() => setSelectedCategory(null)}
                    />
                    
                    <div className={`relative bg-[#1A1A1A] w-full max-w-3xl rounded-[40px] border-t-8 ${selectedCategory.sectionColor || 'border-indigo-600'} border-l border-r border-b border-white/10 overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.8)] animate-in zoom-in-95 duration-300`}>
                        <button 
                            onClick={() => setSelectedCategory(null)}
                            className="absolute top-10 right-10 w-12 h-12 rounded-full bg-white/5 flex items-center justify-center text-gray-500 hover:text-white transition-all hover:rotate-90 hover:bg-white/10"
                        >
                            <X className="w-6 h-6" />
                        </button>
                        ...
                        <div className="p-10 md:p-16 text-left">
                            <div className="flex items-center gap-6 mb-10">
                                <div className={`w-16 h-16 bg-[#262626] rounded-3xl flex items-center justify-center border border-white/5 shadow-inner`}>
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-500 font-bold uppercase tracking-widest text-[10px] mt-1">{ui.freeInBeta}</p>
                                </div>
                            </div>

                            <div className="mb-12">
                                <div className="flex items-center justify-between mb-6">
                                    <p className="text-[11px] text-gray-500 uppercase tracking-widest font-black flex items-center gap-2">
                                        <Terminal className="w-4 h-4 opacity-30" />
                                        Налаштований промпт для ШІ:
                                    </p>
                                </div>
                                <div className="bg-[#0D0D0D] border border-white/10 rounded-3xl p-8 font-mono text-sm md:text-base text-indigo-100/60 leading-relaxed max-h-[350px] overflow-y-auto shadow-inner custom-scrollbar whitespace-pre-line ring-1 ring-white/5">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                ...
                                <button
                                    ...
                                    className={`flex items-center justify-center gap-4 py-6 rounded-[2rem] font-black text-lg transition-all border-2 border-white/10 ${
                                        copied 
